...
<head>
    ...
    <title>Aventones | Panel de Administración</title>
    <meta name="description" content="Gestiona usuarios en el panel de administración de Aventones">
...
<body>
    ...
    <div class="container py-5">
            ...
            <div class="header-box mt-3">
                <div class="header-content d-flex flex-column flex-md-row justify-content-between align-items-center gap-3">
                    <nav class="nav-bar d-flex flex-column flex-sm-row align-items-center gap-3">
                        <a href="{{ route('home') }}" class="hover-grow">Panel</a>
                        <a href="#" class="hover-grow active">Administración</a>
                    </nav>
        ...
        <div class="header-section">
            ...
            <p class="page-subtitle">Gestiona el estado de los usuarios del sistema</p>
        </div>
                        ...
                        <tbody>
                            @foreach ($users as $index => $user)
                                <tr class="table-row">
                                    <td>{{ $index + 1 }}</td>
                                    <td>
                                        <span class="badge-id">{{ $user->cedula }}</span>
                                    </td>
                                    <td>
                                        <div class="user-info">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                    ...
                                    <td>
                                        @if($user->state == 'active' || $user->state == 1)
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            @if($user->state == 'active' || $user->state == 1)
                                                <form action="{{ route('updateUser', $user->cedula) }}" method="POST" class="d-inline">
                                                    @csrf   
                                                    @method('PUT')
                                                    <button type="submit" class="btn-action btn-disable" title="Desactivar Usuario" onclick="return confirm('¿Estás seguro de que deseas desactivar este usuario?')">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                                                            <circle cx="12" cy="12" r="10"></circle>
                                                            <line x1="15" y1="9" x2="9" y2="15"></line>
                                                            <line x1="9" y1="9" x2="15" y2="15"></line>
                                            ...
                                            @else
                                                <form action="{{ route('updateUser', $user->cedula) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('PUT')
                                                    <button type="submit" class="btn-action btn-enable" title="Activar Usuario">
                                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
